...
HRESULT CDXUTMesh10::CreateMesh( ID3D10Device *pDev10, LPCTSTR szFileName, VOID* pData, DWORD dwDataSizeInBytes, D3D10_INPUT_ELEMENT_DESC* playout, UINT cElements, bool bOptimize )
{
    ...
    __try
    {
        ...
        if( SUCCEEDED( pRawMesh->CloneMesh( D3DXMESH_32BIT | D3DXMESH_DYNAMIC, pdecl, pDev9, &pMesh ) ) )
        {
			...
			// IAC: Contrary to Bryan's comment in #3, these lines are necessary.  Otherwise some meshes with
			// multiple materials don't load correctly (Andrei's bonfire model).  Added the D3DLOCK_READONLY 
			// flag because otherwise LockAttributeBuffer zeros the attribute buffer in pMesh under some 
			// circumstances.  (Why?!?)  The READONLY fixes Bryan's SkinnedInstancing app.
            //set the attribute buffer
            DWORD* pdwData = NULL;
            pMesh->LockAttributeBuffer( D3DLOCK_READONLY, &pdwData );
            ...
            {
                // get new attribute table, or old one if we decided not to optimize...
                m_pMesh10->GetAttributeTable( NULL, &m_dwNumAttr );
                SAFE_DELETE_ARRAY( m_pAttr );
                m_pAttr = new D3DX10_ATTRIBUTE_RANGE[m_dwNumAttr];
            ...
            if( pMat )
            {
                ...
                for(unsigned int i=0;i<dwNumMats;i++)
                {
                    m_pMats[i].pTextureFilename = NULL;
                }
                ...
                for( DWORD t = 0; t < m_dwNumAttr; ++t )
                {
                    ...
                    {
                        // copy the string, since the local string will be released and we need to save the filename
                        m_pMats[m_pAttr[t].AttribId].pTextureFilename = new CHAR[MAX_PATH];
                        StringCchCopyA(m_pMats[m_pAttr[t].AttribId].pTextureFilename,MAX_PATH,pMats[m_pAttr[t].AttribId].pTextureFilename);
...
void CDXUTMesh10::Destroy()
{
    ...
    // We allocated an copied in the string, delete it.
    for( DWORD i = 0; i < m_dwNumAttr; ++i )
    {
        SAFE_DELETE_ARRAY( m_pMats[m_pAttr[i].AttribId].pTextureFilename);
    }
