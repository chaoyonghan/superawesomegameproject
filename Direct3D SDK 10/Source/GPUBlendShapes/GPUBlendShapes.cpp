...
void PrintMessage(int level, LPCWSTR fmt, ...)
{
    ...
    if (level == LOG_ERR)
    {
        DXUT_SetPerfMarker(D3DCOLOR_XRGB(255, 0, 0), dest);             // RED
    }
    else if (level == LOG_WARN)
    {
        DXUT_SetPerfMarker(D3DCOLOR_XRGB(0, 255, 255), dest);           // YELLOW
    }
    else // level == LOG_MSG or anything else
    {
        DXUT_SetPerfMarker(D3DCOLOR_XRGB(0, 255, 0), dest);             // GREEN
    }
#endif

}

#define D3D_PERF_EVENT_GENERATOR( color, msg )      CDXUTPerfEventGenerator perfEventGenerator( (color), (msg) )
...
namespace Tools
{
    ...
    HRESULT CreateTexture2DFromFile(LPCSTR texfile, ID3D10Texture2D **ppTex, ID3D10ShaderResourceView **ppView, ID3D10Device* pd3dDevice)
    {
        ...
        D3DX10GetImageInfoFromFile(Tools::PathNameAsWS(texfile), NULL, &SrcInfo, &hr);
        ...
        D3DX10CreateTextureFromFile( pd3dDevice, Tools::PathNameAsWS(texfile), &LoadInfo, NULL, &pTexture, &hr);
        if(FAILED(hr))
          return hr;
...
bool InitEffect(ID3D10Device* pd3dDevice)
{
    ID3D10Blob *pErrors;
    HRESULT  hr= S_OK;
    hr = D3DX10CreateEffectFromFile(Tools::PathNameAsWS(L"GPUBlendShapes.fx"), NULL, NULL, "fx_4_0", D3D10_SHADER_NO_PRESHADER, 0, pd3dDevice, NULL, NULL, &g_Effect, &pErrors, &hr);
...
{
    D3D_PERF_EVENT_GENERATOR( D3DCOLOR_XRGB(128,128,128), L"FrameRender" ); // GREY
    ...
    case RENDER_STREAMOUT:
        {
            D3D_PERF_EVENT_GENERATOR( D3DCOLOR_XRGB(255,255,0), L"StreamOut" ); // YELLOW
            OnD3D10FrameRender_StreamOut( pd3dDevice, fTime, fElapsedTime, pUserContext );
        }
        break;
    case RENDER_BUFFERTEMPLATE:
        {
            D3D_PERF_EVENT_GENERATOR( D3DCOLOR_XRGB(0,255,255), L"BufferTemplate" ); // CYAN
    ...
    const size_t MESHNAME_MAX_LENGTH = 32;
    wchar_t meshName[MESHNAME_MAX_LENGTH] = L"Mesh ";
    const size_t k_counterIndex = wcslen(meshName);
    // Begin static meshes perf marker
    for(int m=0; m<MESHNUM; m++)
    {
        ::_itow_s( m, &(meshName[k_counterIndex] ),
            MESHNAME_MAX_LENGTH - k_counterIndex - 1, 10);
        D3D_PERF_EVENT_GENERATOR( D3DCOLOR_XRGB(128,255,0), meshName ); // BRIGHT GREEN
        ...
        if(g_MeshTable[m].pMesh->numBlendShapes == 0)
        {
            ...
            const size_t STATICMESHNAME_MAX_LENGTH = 32;
            wchar_t staticMeshName[STATICMESHNAME_MAX_LENGTH] = L"static mesh ";
            const size_t k_counterIndex = wcslen(staticMeshName);
            ...
            for(int i=0; i<g_MeshTable[m].numIndexBuffers; i++)
            {
                ...
                // Perf markers for static meshes
                {
                    ::_itow_s( i, &(staticMeshName[k_counterIndex] ),
                        STATICMESHNAME_MAX_LENGTH - k_counterIndex - 1, 10);
                    D3D_PERF_EVENT_GENERATOR( D3DCOLOR_XRGB(0,128,0), staticMeshName ); // GREEN
                    pd3dDevice->DrawIndexed( g_MeshTable[m].pMesh->primGroup[i].indexCount, 0, 0 );
                }
            }
        }
    }
    // End static meshes perf marker



    // Perf: we do not want to profile the UI
    //
    // Stats + UI
    //

    {
        D3D_PERF_EVENT_GENERATOR( D3DCOLOR_XRGB(128,64,0), L"DrawStats" ); // BROWN
        Tools::DrawStats();
    }

    {
        D3D_PERF_EVENT_GENERATOR( D3DCOLOR_XRGB(0,64,128), L"UI Render" ); // NAVY
        g_SampleUI.OnRender( fElapsedTime );
    }

    {
        D3D_PERF_EVENT_GENERATOR( D3DCOLOR_XRGB(64,0,128), L"HUD Render" ); // PURPLE
